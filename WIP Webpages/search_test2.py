...
if search_query:
    ...
    # Query the database for matching faculty names
    try:
        query = "SELECT faculty_name FROM faculty WHERE faculty_name ILIKE %s LIMIT 10;"  # Adjust to select only faculty_name
        cursor.execute(query, ('%' + search_query + '%',))
        rows = cursor.fetchall()

        if rows:
            print("<ul>")  # Using an unordered list to display names
            for row in rows:
                print(f"<li>{row[0]}</li>")  # Print the faculty name (first column)
            print("</ul>")
        else:
            print("<p>No results found for your search.</p>")
    except Exception as e:
        print(f"<p>Error querying the database: {str(e)}</p>")
else:
